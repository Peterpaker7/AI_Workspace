...
class EDA:
    ...
    def probability_distribution(df, columns=None, bins=30, figsize=(8, 5)):
        ...
        for col in columns:
            ...
            frequency_table["Class Interval"] = pd.cut(
                data,
                bins=bins
            ).value_counts(sort=False).index.astype(str)

            frequency_table["Frequency"] = pd.cut(
                data,
                bins=bins
            ...
            # Quartiles
            plt.axvline(q1, color='brown', linestyle='-.', linewidth=2,
                        label='Q1')

            plt.axvline(q2, color='black', linestyle='-.', linewidth=2,
                        label='Q2')

            plt.axvline(q3, color='brown', linestyle='-.', linewidth=2,
                        label='Q3')

            # Shade the IQR region
            plt.axvspan(q1, q3, alpha=0.2, color='cyan',
                        label='IQR')
